...
baseline = load_data_meta3split(
    cwd + "[baseline with breakdown] exp09-06-2021-19-21-59/"
)
...
gap_4split = (
    pd.DataFrame(data4split[3]["overall"]).rolling(trainwindow).mean()
    - pd.DataFrame(data4split[2]["overall"]).rolling(trainwindow).mean()
)
gap_3split = (
    pd.DataFrame(baseline[3]["overall"]).rolling(trainwindow).mean()
    - pd.DataFrame(baseline[1]["overall"]).rolling(trainwindow).mean()
)
...
plt.legend(loc="upper center", bbox_to_anchor=(-0.7, -0.1), frameon=False)
