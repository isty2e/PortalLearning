...
class portal_four_universe_Ensemble(nn.Module):
    ...
    def forward(self, batch_data):
        batch_chem_graphs_spt, batch_protein_tokenized_spt = get_repr_DTI_illum(
            batch_data, self.args
        )
        ...
        return (
            batch_logits1.detach().cpu(),
            batch_logits2.detach().cpu(),
            batch_logits3.detach().cpu(),
        )
...
def get_repr_DTI_illum(batch_data, args):
    ...
    chem_graphs_loader = DataLoader(
        chem_graph_list, batch_size=len(chem_graph_list), shuffle=False
    )
    for batch in chem_graphs_loader:
        chem_graphs = batch
    #  . . . .  proteins  . . . .
    protein_tokenized = torch.tensor(batch_data["protein"]).repeat(
        len(chem_graph_list), 1
    )
    #  . . . .  to return  . . . .
