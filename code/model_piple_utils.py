...
class distMtx_core_module(nn.Module):
    def __init__(self, dim=20, num_class=3, feat_mode="attentive-pool"):
...
class DTI_distMtx_classifier(nn.Module):
    def __init__(
        ...
        pred_mode="binary",
        protein_descriptor=None,
        chem_descriptor=None,
        frozen="whole",
        cwd=None,
        ...
        self.pred_DistMtx = distMtx_core_module(
            dim=300, num_class=num_class, feat_mode=feat_mode
        )
        ...
        self.prot_transform = torch.nn.Linear(
            dim, 300
        )  # transform to same dim as contextpred
        self.frozen = frozen
    ...
    def forward(self, batch_input, device):
        ...
        if self.frozen == "whole":
            with torch.no_grad():
                embed_full_prot = self.protein_descriptor(
                    batch_input["tokenized-padded"]
                )[0][
                    :, 1:-1, :
                ]  # with beining and end tokens
        else:
            # print('updating..')
            embed_full_prot = self.protein_descriptor(batch_input["tokenized-padded"])[
                0
            ][
                :, 1:-1, :
            ]  # with beining and end tokens
        embed_bs_prot = torch.bmm(
            batch_input["binding site selection matrix|prot"], embed_full_prot
        )
        embed_bs_prot_T = self.prot_transform(embed_bs_prot)
        # ---------------chemical embedding -------------------
        chem_graphs_in = batch_input["chem graph loader"]
        embed_full_chem = self.chem_decriptor(
            chem_graphs_in.x, chem_graphs_in.edge_index, chem_graphs_in.edge_attr
        )
        graph_repr_masked, mask_graph = to_dense_batch(
            embed_full_chem, chem_graphs_in.batch
        )
        embed_bs_chem = torch.bmm(
            batch_input["binding site selection matrix|chem"], graph_repr_masked
        )
