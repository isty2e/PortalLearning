...
from data_tool_box import load_pkl
from ligand_graph_features import mol_to_graph_data_obj_simple
from utils_ensemble import portal_four_universe_single
from utils_MAML import load_DISAE
...
def get_result(truescore, dockscore):
    ...
    prec, recall, thresholds2 = precision_recall_curve(
        truescore, dockscore, pos_label=1
    )
...
def scores(portal):
    roc = round(auc(portal[0], portal[1]), 2)
    pr = round(auc(portal[4], portal[3]), 2)
...
for protein in dark_proteins:
    ...
    for j in range(bound):
        ...
        batch_drug_graph = graph_list[
            j * args["batchsize"] : (j + 1) * args["batchsize"]
        ]
        ...
        batch_drug_name.index = ikey_list[
            j * args["batchsize"] : (j + 1) * args["batchsize"]
        ]
