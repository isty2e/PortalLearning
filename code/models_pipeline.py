...
from models_MAML import AttentivePooling, EmbeddingTransform
from resnet import ResnetEncoderModel


class DTI_pipeline(nn.Module):
    def __init__(self, all_config=None, protein_descriptor=None, chem=None):
    ...
    def forward(self, batch_protein_tokenized, batch_chem_graphs, **kwargs):
        ...
        if self.all_config["protein_descriptor"] == "DISAE":
            ...
            batch_protein_repr_transformed = self.resnet(
        ...
        ((chem_vector, chem_score), (prot_vector, prot_score)) = (
            self.attentive_interaction_pooler(
                batch_chem_graphs_repr_pooled, batch_protein_repr_transformed
            )
