...
from data_tool_box import get_repr_DTI, load_json
...
def load_DISAE(cwd):
    ...
    albertconfig = AlbertConfig.from_pretrained(
        "../" + all_config["DISAE"]["albertconfig"]
    )
    m = AlbertForMaskedLM(config=albertconfig)
    m = load_tf_weights_in_albert(
        m, albertconfig, "../" + all_config["DISAE"]["albert_pretrained_checkpoint"]
    )
    prot_descriptor = m.albert
    prot_tokenizer = BertTokenizer.from_pretrained(
        "../" + all_config["DISAE"]["albertvocab"]
    )
    return prot_descriptor, prot_tokenizer


def get_repr_DTI_MAML(
    batch_data_pertask, mode, prot_tokenizer, chem_dict, protein_dict, args
):
    batch_chem_graphs_spt, batch_protein_tokenized_spt = get_repr_DTI(
        batch_data_pertask[mode], prot_tokenizer, chem_dict, protein_dict, "DISAE"
    )
    ...
    y_spt = torch.LongTensor(list(batch_data_pertask[mode]["Activity"].values)).to(
        args["device"]
    )
    return batch_chem_graphs_spt, batch_protein_tokenized_spt, y_spt


def core_batch_prediction_MAML(
    batch_data_pertask,
    args,
    tokenizer,
    mode,
    ...
    batch_chem_graphs_spt, batch_protein_tokenized_spt, y_spt = get_repr_DTI_MAML(
        batch_data_pertask, mode, tokenizer, chem_dict, protein_dict, args
    )
...
def core_batch_prediction_0shot(
    batch_data_pertask,
    args,
    tokenizer,
    loss_fn,
    ...
    batch_chem_graphs_spt, batch_protein_tokenized_spt, y_spt = get_repr_DTI_0shot(
        batch_data_pertask, tokenizer, chem_dict, protein_dict, args
    )
